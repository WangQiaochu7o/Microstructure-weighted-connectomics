...
class AnalysisConfig:
    ...
    def __init__(self):
        # MODIFICATION: Update base paths for modularity results
        self.base_path = "/home/localadmin/Desktop/SP2"
        self.modularity_results_path = "/home/localadmin/Desktop/SP2/84output/group_averaged_output/mod"
        ...
        self.gamma_for_plotting = 0.8 # <-- CHANGE THIS VALUE TO PLOT A DIFFERENT GAMMA
...
def generate_dynamic_colormap(n_communities):
    if n_communities == 0: return ListedColormap(['#FFFFFF']) # Use white for no communities
    if n_communities <= 10:
...
def map_data_to_surface(data_values, lh_labels, rh_labels, lh_names, rh_names):
    ...
    for connectome_idx, data_value in enumerate(data_values):
        ...
        if fs_node_idx in node_to_region and data_value != 0:
            full_region_name = node_to_region[fs_node_idx]
            hemi_prefix, region_name = full_region_name.split('.', 1)
...
def collect_gamma_sweep_data():
    ...
    for metric in config.metrics:
        # MODIFICATION: Path now points to the 'best' folder inside each metric's directory
...
# --- MODIFICATION: Rewritten to fetch assignments for a specific gamma ---
    ...
    for metric in config.metrics:
        ...
        if os.path.exists(file_path):
            try:
                ...
                # Ensure array is the correct size for the atlas
                full_assignments = np.zeros(84, dtype=int)
                num_nodes_in_file = len(assignments)
                if num_nodes_in_file > 84:
                    print(f"  Warning for {metric}: File has {num_nodes_in_file} nodes, trimming to 84.")
                    full_assignments = assignments[:84]
                else:
...
def create_gamma_sweep_plots(gamma_sweep_data, optimal_data, output_dir):
    ...
    plt.savefig(os.path.join(output_dir, "modularity_gamma_sweep_aggregated.png"), dpi=300, bbox_inches='tight')
    plt.close()
...
def create_community_brain_visualization(community_data, output_dir, target_gamma):
    ...
    # MODIFICATION: Dynamic title including the selected gamma
    # fig.suptitle(f'Modularity Assignment (γ={target_gamma:.1f}) on Desikan-Killiany Atlas', fontsize=22, fontweight='bold', y=0.98)
    
    # MODIFICATION: Dynamic filename for the output image
    output_filename = os.path.join(output_dir, f"community_assignments_aggregated_gamma_{target_gamma:.1f}.png")
...
def main_plotting():
    print("="*60)
    print("STARTING FLEXIBLE PLOTTING SCRIPT")
