...
class AnalysisConfig:
    ...
    def __init__(self):
        ...
        self.output_path = "/home/localadmin/Desktop/SP2/84output/group_averaged_output/ward"
        self.freesurfer_home = '/home/localadmin/freesurfer'
        self.subjects_dir = os.path.join(self.freesurfer_home, 'subjects')
        # MODIFICATION: Add 'BINARY_NOS' to the list of metrics to process
        ...
        self.hierarchical_linkage_methods = ['ward']
...
metric_colors = {
    ...
    'BINARY_NOS': '#000000' # MODIFICATION: Add black color for the binary network
}
metric_name_map_latex = {
    'NOS': r'$nos$', 'RK_DKI': r'$rk$', 'FA_DTI': r'$fa$',
    'RD_DTI_reciprocal': r'$1/rd$', 'DePerp_SMI_reciprocal': r'$1/D_{\perp e}$',
    'F_SMI': r'$f$', 'R1': r'$R1$',
    'BINARY_NOS': r'$binary$' # MODIFICATION: Add a label for the binary network
}
...
def compute_uncentered_correlation_distance(W):
    """Compute distance matrix using uncentered Pearson correlation"""
...
def compute_hierarchical_clustering_analysis(W):
    """Compute hierarchical clustering using only the 'ward' method."""
    ...
    try:
        ...
        linkage_matrix = perform_hierarchical_clustering(distance_matrix, 'ward')
...
def get_dk_atlas_mapping():
    ...
    for i, region in enumerate(cortical_regions): node_to_region[50 + i] = f"rh.{region}"
    return node_to_region

def generate_dynamic_colormap(n_communities):
    if n_communities <= 10:
        return ListedColormap(plt.get_cmap('tab10').colors[:n_communities])
...
def load_freesurfer_dk_atlas():
    try:
        ...
        print(f"Error loading DK atlas: {e}")
...
def map_data_to_surface_dk(data_values, lh_labels, rh_labels, lh_names, rh_names):
    ...
    node_to_region = get_dk_atlas_mapping()
...
def create_aggregated_silhouette_sweep_plot(all_sweep_data, all_optimal_data, output_dir):
    ...
    ax.set_title('Silhouette Score vs. Number of Clusters (Minimum Variance Method, Desikan-Killiany Atlas)', fontsize=16, fontweight='bold')
    ...
    # MODIFICATION: Update filename to reflect inclusion of binary network
    output_file = os.path.join(output_dir, "hierarchical_silhouette_sweep_aggregated_84regions_with_binary.png")
...
def create_brain_visualization_for_cluster_numbers(metric, linkage_matrix, output_dir):
    """Creates a grid of brain visualizations showing how clusters evolve with k."""
    print(f"\nCreating brain visualization grid for {metric} (DK-84 atlas)...")
    
    plt.rcParams['mathtext.fontset'] = 'cm'
    lh_labels, rh_labels, lh_names, rh_names = load_freesurfer_dk_atlas()
    if lh_labels is None:
        print("  Failed to load FreeSurfer DK atlas, skipping visualization.")
        return
    ...
    for row, n_clusters_k in enumerate(cluster_numbers_to_plot):
        ...
        for col, (view, hemi, view_title) in enumerate(views):
            ...
            try:
                ...
                temp_file = f"{output_dir}/temp_{metric}_{n_clusters_k}_84regions.png"
    ...
    fig.suptitle(f'Hierarchical Clustering on Desikan-Killiany Atlas ({latex_metric_name})', 
                 fontsize=18, fontweight='bold', y=0.98)
    
    output_file = f"{output_dir}/hierarchical_assignments_by_k_{metric}_84regions.png"
...
def main():
    """Main function for hierarchical clustering analysis (DK-84)."""
    print("="*80)
    print("Hierarchical Clustering Analysis - ward Method (84-region Desikan-Killiany)")
    print("="*80)
    ...
    for metric in config.metrics:
        # MODIFICATION: Determine file path based on whether it is the binary metric
        ...
        if W.shape[0] != 84:
            print(f"  [Error] Connectome matrix is not 84x84. Shape: {W.shape}. Skipping.")
            continue
        
        results = compute_hierarchical_clustering_analysis(W)
        ...
        if results and results['success']:
            ...
            # --- ADDITION: Save the optimal cluster assignments ---
            print(f"    Saving optimal cluster assignments for metric '{metric}'...")
            optimal_k = results['optimal_n_clusters']
            optimal_assignments = fcluster(linkage_matrix, optimal_k, criterion='maxclust')
            assignment_df = pd.DataFrame({
                'region_index': range(1, len(optimal_assignments) + 1),
                'cluster_id': optimal_assignments
            })
            output_assignment_file = os.path.join(
                config.output_path, 
                f"hierarchical_optimal_assignments_{metric}_84regions.csv"
            )
            assignment_df.to_csv(output_assignment_file, index=False)
            print(f"    Assignments saved to: {output_assignment_file}")
