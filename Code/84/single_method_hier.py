...
class AnalysisConfig:
    ...
    def __init__(self):
        ...
        self.output_path = "/home/localadmin/Desktop/SP2/84output/group_averaged_output/single"
        ...
        self.hierarchical_linkage_methods = ['single']
...
def perform_hierarchical_clustering(distance_matrix, linkage_method='single'):
...
def compute_hierarchical_clustering_analysis(W):
    """Compute hierarchical clustering using only the 'single' method."""
    ...
    try:
        print("      Running single linkage...")
        linkage_matrix = perform_hierarchical_clustering(distance_matrix, 'single')
...
# --- NEW FUNCTION: Creates one aggregated plot for silhouette scores ---
def create_aggregated_silhouette_sweep_plot(all_sweep_data, all_optimal_data, output_dir):
    ...
    ax.set_title('Silhouette Score vs. Number of Clusters (Single Method, Desikan-Killiany Atlas)', fontsize=16, fontweight='bold')
    ...
    fig.subplots_adjust(right=0.75) # Make space for legend and text
    output_file = os.path.join(output_dir, "hierarchical_silhouette_sweep_aggregated_84regions.png")
...
def create_brain_visualization_for_cluster_numbers(metric, linkage_matrix, output_dir):
    ...
    cluster_numbers_to_plot = range(2, 11) # Plot for k=2 through k=6
...
def main():
    ...
    print("="*80)
    print("Hierarchical Clustering Analysis - single Method (84-region Desikan-Killiany)")
    print("="*80)
