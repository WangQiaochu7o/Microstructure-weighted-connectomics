...
def get_corrected_a2009s_atlas_mapping():
    ...
    Structure according to fs_a2009s.txt:
    - Node 0: Unknown
    - Nodes 1-74: Left hemisphere cortical (74 regions)
    - Node 75: Left cerebellum
    - Nodes 76-82: Left subcortical (7 regions)
    - Nodes 83-89: Right subcortical (7 regions)
    - Nodes 90-163: Right hemisphere cortical (74 regions)
    - Node 164: Right cerebellum
    """
    
    # Destrieux atlas cortical region names (74 regions per hemisphere)
    # Extracted from fs_a2009s.txt, removing 'ctx_lh_' and 'ctx_rh_' prefixes
    cortical_regions = [
    ...
    # Nodes 1-74: Left hemisphere cortical regions
    for i, region in enumerate(cortical_regions):
        node_idx = i + 1  # Start from node 1
        node_to_region[node_idx] = f"lh.{region}"
    
    # Nodes 90-163: Right hemisphere cortical regions
    for i, region in enumerate(cortical_regions):
        node_idx = 90 + i  # Start from node 90
        node_to_region[node_idx] = f"rh.{region}"
    
    # Note: Subcortical and cerebellar regions are excluded from mapping
...
def map_data_to_surface(data_values, lh_labels, rh_labels, lh_names, rh_names):
    ...
    for connectome_idx, data_value in enumerate(data_values):
        fs_node_idx = connectome_idx + 1  # Convert 0-163 to 1-164
...
def collect_group_averaged_hierarchical_assignments():
    ...
    Using 164-region atlas structure
        ...
        if os.path.exists(partition_file):
            try:
                ...
                if len(df.columns) >= 2:
                    ...
                    # Create full assignment array for 164 regions
...
def create_hierarchical_brain_visualization(cluster_data, output_dir):
    ...
    Updated for 164 regions
    """
    print("="*60)
    print("CREATING HIERARCHICAL CLUSTER BRAIN VISUALIZATIONS (GROUP AVERAGED - 164 REGIONS)")
    print("="*60)
    ...
    output_file = f"{output_dir}/hierarchical_clustering_group_averaged_164_regions.png"
...
def save_hierarchical_assignments(cluster_data, output_dir):
    """
    Save detailed hierarchical cluster assignment data for 164 regions (group averaged)
    """
    ...
    for metric in config.metrics:
        if metric in cluster_data:
            ...
            for node_idx, cluster_id in enumerate(assignments):
                if node_idx < 164:  # Only process 164 regions
    ...
    assignment_file = f"{output_dir}/hierarchical_assignments_group_averaged_164_regions.csv"
    ...
    for metric in config.metrics:
        if metric in cluster_data:
            ...
            summary_data.append({
                ...
                'Unassigned_Nodes': 164 - num_assigned_nodes,
                'Cluster_IDs': ','.join(map(str, unique_clusters))
            })
    
    summary_df = pd.DataFrame(summary_data)
    summary_file = f"{output_dir}/hierarchical_summary_group_averaged_164_regions.csv"
...
def process_connectome_hierarchical_analysis(connectome_path, output_dir, metric):
    ...
    try:
        ...
        if W.shape[0] != 164:
            print(f"  [WARNING] Expected 164x164 matrix, got {W.shape}")
...
def main():
    ...
    """
    print("="*80)
    print("HIERARCHICAL CLUSTERING ANALYSIS - GROUP AVERAGED CONNECTOMES (164 REGIONS)")
    print("="*80)
