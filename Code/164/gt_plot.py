...
output_path = "/home/localadmin/Desktop/SP2/164output/pro_output"
...
def collect_data_by_weighted_metric():
    ...
    for weighted_metric in metrics:
        ...
        for subject in aim1_subjects:
            file_path = f"{base_path}/Aim1_sub-{subject}/connectomes_v2/dual_thresholded_connectomes/analysis_v2/{weighted_metric}/z_scores_all.csv"
        ...
        for subject in aim2_subjects:
            file_path = f"{base_path}/Aim2_sub-{subject}/connectomes_v2/dual_thresholded_connectomes/analysis_v2/{weighted_metric}/z_scores_all.csv"
            if os.path.exists(file_path):
                try:
                    df = pd.read_csv(file_path)
                    # For each network metric in this file
...
def collect_swp_data_by_weighted_metric():
    ...
    for weighted_metric in metrics:
        ...
        # Process Aim 1
        for subject in aim1_subjects:
            file_path = f"{base_path}/Aim1_sub-{subject}/connectomes_v2/dual_thresholded_connectomes/analysis_v2/{weighted_metric}/small_world_propensity.csv"
            if os.path.exists(file_path):
                try:
                    df = pd.read_csv(file_path)
        ...
        # Process Aim 2
        for subject in aim2_subjects:
            file_path = f"{base_path}/Aim2_sub-{subject}/connectomes_v2/dual_thresholded_connectomes/analysis_v2/{weighted_metric}/small_world_propensity.csv"
            if os.path.exists(file_path):
                try:
                    df = pd.read_csv(file_path)
                    if 'SWP' in df.columns:
...
def create_network_metric_plot(network_metric, weighted_metric_data, output_dir, corrected_z_threshold):
    ...
    ax.set_title(plot_title + "-Destrieux", fontsize=14, fontweight='bold')
...
def create_swp_plot(swp_data, output_dir, corrected_z_threshold):
    ...
    ax.set_title('Small World Propensity-Destrieux', fontsize=14, fontweight='bold')
    # Style similar to reference
    ax.grid(True, alpha=0.3)
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
